...
<fieldset>
    ...
    <div style="width: 680px; height:500px; overflow:auto">
        <table id="memberListTable">
            ...
            %>
            <tr>
                <td><%=member.getTitle() + " " + member.getFirstName() + " " + member.getLastName()%>
                </td>
                <td><%=member.getStreetAddress() + ", " + member.getCity() + "<br>" + member.getState() + " " + member.getPostcode() + " " + member.getCountry()%>
                </td>
                <td><%=member.getEmail()%>
                </td>
                <td><%=member.getPhone()%>
                </td>
                <td><%=member.getInstitution()%>
                </td>
                <td><%=member.getResearchInterest()%>
                </td>
                <td><%=member.getNewsletterPref()%>
                </td>
                <td><%=member.getAcrsEmailListFlag()%>
                </td>
                <td><%=member.getMembershipType()%>
                </td>
                <td><%=member.getRenewalFlag()%>
                </td>
                <td><%=member.getMembershipAmount() + "0"%>
                </td>
                <td><%=member.getRegistrationDate()%>
                </td>
                <td><%=member.getPaypalStatus()%>
                </td>
                <td><%=paypalRefStr + paypalDetails%>
                </td>
                <td>
                    <input type="button" value="EDIT"
                           onClick="self.location = '<portlet:renderURL><portlet:param name="cmd" value="EDIT" /><portlet:param name="memberId" value="<%= String.valueOf(member.getId()) %>" /></portlet:renderURL>';"/>
                </td>
            </tr>
            <% } %>
        </table>
        ...
        <fieldset>
            ...
            <div>
                ...
                <select name="title" id="title">
                    ...
                    <option selected="selected"
                            value="<%= isEdit ? editMember.getTitle() : "Mr" %>"><%= isEdit ? editMember.getTitle() : "Mr" %>
                    </option>
                </select>
            </div>
            ...
            <div>
                ...
                <select name="country" id="country">
                    <option selected="selected"
                            value="<%= isEdit ? editMember.getCountry() : "Australia" %>"><%= isEdit ? editMember.getCountry() : "Australia" %>
                    </option>
            ...
            <div>
                ...
                <textarea rows="3" cols="70" name="institution" id="institution">
                    <%= isEdit ? editMember.getInstitution() : emptyStr %>
                </textarea>
            </div>
            ...
            <div>
                ...
                <textarea rows="3" cols="70" name="researchInterest" id="researchInterest">
                    <%= isEdit ? editMember.getResearchInterest() : emptyStr %>
                </textarea>
            </div>
            <br>
            ...
            <div>
                ...
                <input class="inputCheckbox" type="checkbox" name="newsletterPref"
                       value="PDF" <%= isEdit ? (editMember.getNewsletterPref().startsWith("PDF") ? " checked" : emptyStr) : emptyStr %>/>PDF
                <br>
                <input class="inputCheckbox" type="checkbox" name="newsletterPref2"
                       value="Hard Copy" <%= isEdit ? (editMember.getNewsletterPref().endsWith("Hard Copy") ? " checked" : emptyStr) : emptyStr %> />Hard
                Copy
            ...
            <div>
                ...
                <input class="radioCheckbox" type="radio" name="membershipType"
                       value="Full" <%= isEdit ? (editMember.getMembershipType().equals("Full") ? " checked" : emptyStr) : emptyStr %>/>
                Full ($50.00)<br/>
                <input class="radioCheckbox" type="radio" name="membershipType"
                       value="Student" <%= isEdit ? (editMember.getMembershipType().equals("Student") ? " checked" : emptyStr) : emptyStr %>/>
                Student ($30.00)<br/>
                <input class="radioCheckbox" type="radio" name="membershipType"
                       value="FiveYear" <%= isEdit ? (editMember.getMembershipType().equals("FiveYear") ? " checked" : emptyStr) : emptyStr %>/>
                5 Year Full ($200.00)<br/>
                <input class="radioCheckbox" type="radio" name="membershipType"
                       value="Test" <%= isEdit ? (editMember.getMembershipType().equals("Test") ? " checked" : emptyStr) : emptyStr %>/>
                Test ($5.00)<br/>
                ...
                <%} else { %>
                <div>
                    There is a $10.00 discount on Full and Student single year memberships for applications received
                    between 1 January and 28 February each year. Memberships are valid until the end of the calendar
                    year.
                </div>
            </div>
            <%} %>
            ...
            <div>
                ...
                <input class="inputCheckbox" type="checkbox" name="renewalFlag" value="Y"
                        <%= isEdit ? (editMember.getRenewalFlag().equals("Y") ? " checked" : emptyStr) : emptyStr %>/>I
                am renewing my previous membership.<br>
            </div>

            <div>
                <label for="acrsEmailListFlag">&nbsp;</label>
                <input class="inputCheckbox" type="checkbox" name="acrsEmailListFlag" value="Y"
                        <%= isEdit ? (editMember.getAcrsEmailListFlag().equals("Y") ? " checked" : emptyStr) : emptyStr %>/>I
                would like to subscribe to the ACRS Email List.<br>
            </div>
            ...
            <div>
                ...
                <div>
                    For all new ACRS members your membership will need to be approved by Council. If for some reason
                    your application is deemed inappropriate, a full refund will be given.
                </div>
                <label for="submit"><br></label>
                <input type="submit" name="submit" value="Save Details and Proceed to Paypal >"
                       onClick="self.location = '<portlet:renderURL><portlet:param name="cmd" value="SUBMIT"/></portlet:renderURL>';"/>
...
<fieldset>
    ...
    <table id="applicantDetailsTable">
        <tr>
            <td class="applLabel">Name</td>
            <td class="applData"><%=newMember.getTitle() + " " + newMember.getFirstName() + " " + newMember.getLastName()%>
            </td>
        </tr>
        <tr>
            <td class="applLabel">Email</td>
            <td class="applData"><%=newMember.getEmail()%>
            </td>
        </tr>
        <tr>
            <td class="applLabel">Phone</td>
            <td class="applData"><%=newMember.getPhone()%>
            </td>
        </tr>
        <!--
...
-->
        <tr>
            <td class="applLabel">Membership Type</td>
            <td class="applData"><%=newMember.getMembershipType()%>
            </td>
        </tr>
        <tr>
            <td class="applLabel">Membership Amount</td>
            <td class="applData"><%=newMember.getMembershipAmount() + "0"%>
            </td>
        </tr>
    </table>
    ...
    <p><b><%=paypalItemName + ": $" + newMember.getMembershipAmount() + "0" %>
    </b></p>
    ...
    <form action="https://www.paypal.com/cgi-bin/webscr" method="post">
        ...
        <input type="image" src="https://www.paypal.com/en_AU/i/btn/btn_paynowCC_LG.gif" border="0" name="submit"
